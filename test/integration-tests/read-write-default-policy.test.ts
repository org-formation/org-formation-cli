...
describe('when writing default template', () => {
    ...
    beforeEach(async () => {
        ...
        fs.writeFileSync('./test/data/sample-org.default-template.yml', template.template);
        template.state.save('./test/data/sample-org.default-state.json');
    });
    ...
    it('then bindings all evaluate to action none', async () => {
        const result = TemplateRoot.createFromContents(template.template);
        const binding = new Binder(result, template.state);
